...
try {
  ...
  throw new Error(`navigation_failed: could not reach ${sel_login_entry_url} — ${e.message}`);
}
...
try {
  ...
  throw new Error(`login_failed: could not find or fill the login form — ${e.message}`);
}
...
try {
  ...
  throw new Error(`login_failed: form submitted but authentication timed out — ${e.message}`);
}
...
try {
  await page.route(api_transfer_detail, async (route) => {
    try {
      ...
      let json = null;
      try { json = JSON.parse(body.toString('utf-8')); } catch {}
  ...
  throw new Error(`route_setup_failed: could not install transferDetail interceptor — ${e.message}`);
}
...
try {
  ...
  if (capturedMovements === null) throw new Error('getMovements API no detectada en 15s');
  movements = capturedMovements;
} catch (e) {
  throw new Error(`movements_fetch_failed: could not load the Ingresos transaction list — ${e.message}`);
}
...
if (context.lastExternalId && String(movements[0].id) === String(context.lastExternalId)) return [];
...
try {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (context.lastExternalId && String(movement.id) === String(context.lastExternalId)) break;
  ...
  throw new Error(`detail_extraction_failed: error iterating transaction rows — ${e.message}`);
}
...
for (const m of movements) {
  try {
    // Cortar al llegar a un movimiento ya registrado — los siguientes también lo están
    if (context.lastExternalId && String(m.id) === String(context.lastExternalId)) break;
